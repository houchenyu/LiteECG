...
from config import SE_ECGNET_CONFIG
...
class SE_ECGNet(nn.Module):
    def __init__(self, config=SE_ECGNET_CONFIG):
        super(SE_ECGNet, self).__init__()
        struct = config['struct']
        num_classes = config['num_classes']
        self.conv = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(1, 50), stride=(1, 2), padding=(0, 0), bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.bn = nn.BatchNorm2d(32)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Linear(in_features=256 * len(struct), out_features=num_classes)
        self.block1 = self._make_layer(in_channels=32, out_channels=32, kernel_size=(1, 15), stride=(1, 2), block=ResBlock2d, blocks=3, padding=(0, 7))
        self.block2_list = nn.ModuleList()
        self.block3_list = nn.ModuleList()
        for i, kernel_size in enumerate(struct):
            block2 = self._make_layer(in_channels=32, out_channels=32, kernel_size=kernel_size, stride=(1, 1), block=ResBlock2d, blocks=4, padding=(0, 1 * (i + 1)))
            block3 = self._make_layer(in_channels=256, out_channels=256, kernel_size=kernel_size[1], stride=2, block=ResBlock1d, blocks=4, padding=1 * (i + 1))
    ...
    def _make_layer(self, in_channels, out_channels, kernel_size, stride, block, blocks, padding=(0, 0)):
        ...
        if blocks == 3:
            downsample = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=(1, 1), stride=(1, 2), padding=(0, 0))
            )
        if block == ResBlock1d:
            downsample = nn.Sequential(
                nn.BatchNorm1d(num_features=in_channels),
                nn.ReLU(inplace=True),
                nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=2, padding=0)
            )
    ...
    def forward(self, x, info=None):
        ...
        if x.shape[1] != 8:
            ...
            x = x.repeat(1, 8, 1)
        out = x.unsqueeze(1)  # [B, 1, 8, T]
        out = self.conv(out)
        out = self.bn(out)
        out = self.relu(out)
        out = self.block1(out)
        out_sep = []
        for i in range(len(SE_ECGNET_CONFIG['struct'])):
            sep = self.block2_list[i](out)
            sep = sep.reshape(sep.shape[0], -1, sep.shape[3])
